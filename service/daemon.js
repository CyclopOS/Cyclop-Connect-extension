...
/**
 * Class representing the CyclopConnect service daemon.
 */
const Service = GObject.registerClass({
    GTypeName: 'CyclopConnectService',
}, class Service extends Gtk.Application {

    _init() {
        super._init({
            application_id: 'org.gnome.Shell.Extensions.CyclopConnect',
            flags: Gio.ApplicationFlags.HANDLES_OPEN,
            resource_base_path: '/org/gnome/Shell/Extensions/CyclopConnect',
        });

        GLib.set_prgname('cyclopconnect');
        GLib.set_application_name('CyclopConnect');
    ...
    _preferences() {
        Gio.Subprocess.new(
            [`${Config.PACKAGE_DATADIR}/cyclopconnect-preferences`],
    ...
    notify_error(error) {
        try {
            ...
            notif.set_title(`CyclopConnect: ${error.name.trim()}`);
            notif.set_body(body);
            notif.set_icon(icon);
    ...
    vfunc_open(files, hint) {
        ...
        for (let file of files) {
            ...
            try {
                ...
                logError(e, `CyclopConnect: Opening ${file.get_uri()}`);
            }
        }
    }
    ...
    _cliAction(id, name, parameter = null) {
        ...
        Gio.DBus.session.call_sync(
            ...
            `/org/gnome/Shell/Extensions/CyclopConnect/Device/${id}`,
            'org.gtk.Actions',
            'Activate',
    ...
    _cliListDevices(full = true) {
        ...
        for (let object of Object.values(variant)) {
            ...
            device = object['org.gnome.Shell.Extensions.CyclopConnect.Device'];
    ...
    _cliNotify(id, options) {
        ...
        if (options.contains('notification-icon')) {
            ...
            icon = new Gio.ThemedIcon({
                name: 'org.gnome.Shell.Extensions.CyclopConnect',
            });
        }
    ...
    vfunc_handle_local_options(options) {
        try {
            if (options.contains('version')) {
                print(`CyclopConnect ${Config.PACKAGE_VERSION}`);
                return 0;
            }
