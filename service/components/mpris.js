...
var Player = GObject.registerClass({
    GTypeName: 'CyclopConnectMediaPlayerInterface',
    Properties: {
    ...
    get DesktopEntry() {
        if (this._DesktopEntry === undefined)
            return 'org.gnome.Shell.Extensions.CyclopConnect';
...
const PlayerProxy = GObject.registerClass({
    GTypeName: 'CyclopConnectMPRISPlayer',
...
var Manager = GObject.registerClass({
    GTypeName: 'CyclopConnectMPRISManager',
    Signals: {
        'player-added': {
    ...
    async _loadPlayers() {
        try {
            ...
            for (let i = 0, len = names.length; i < len; i++) {
                ...
                if (!name.includes('CyclopConnect'))
     ...
     *
     * @param {string} identity - A player name
     * @return {CyclopConnectMPRISPlayer|null} A player or %null
     */
    getPlayer(identity) {
        for (let player of this._players.values()) {
            if (player.Identity === identity)
