...
var Device = GObject.registerClass({
    ...
    _init(identity) {
        ...
        this.settings = new Gio.Settings({
            ...
            path: `/org/gnome/shell/extensions/cyclopconnect/device/${this.id}/`,
        });
    ...
    get encryption_info() {
        ...
        return _('%s Fingerprint:').format(this.name) + '\n' +
            remoteFingerprint + '\n\n' +
            _('%s Fingerprint:').format('CyclopConnect') + '\n' +
            localFingerprint;
    }
    ...
    launchProcess(args, cancellable = null) {
        if (this._launcher === undefined) {
            ...
            this._launcher.setenv('CYCLOPCONNECT', application, false);
            this._launcher.setenv('CYCLOPCONNECT_DEVICE_ID', this.id, false);
            this._launcher.setenv('CYCLOPCONNECT_DEVICE_NAME', this.name, false);
            this._launcher.setenv('CYCLOPCONNECT_DEVICE_ICON', this.icon_name, false);
            this._launcher.setenv(
                'CYCLOPCONNECT_DEVICE_DBUS',
