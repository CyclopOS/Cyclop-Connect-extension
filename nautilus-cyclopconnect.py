"""
nautilus-cyclopconnect.py - A Nautilus extension for sending files via GSConnect.
...
SERVICE_NAME = 'org.gnome.Shell.Extensions.CyclopConnect'
SERVICE_PATH = '/org/gnome/Shell/Extensions/CyclopConnect'

# Init gettext translations
LOCALE_DIR = os.path.join(GLib.get_user_data_dir(),
                          'gnome-shell', 'extensions',
                          'cyclop-connect@cyclop-os.com', 'locale')
...
try:
    ...
    print('CyclopConnect: {0}'.format(e.strerror))
    i18n = gettext.translation(SERVICE_NAME,
                               localedir=LOCALE_DIR,
                               fallback=True)
    _ = i18n.gettext
...
class CyclopConnectShareExtension(GObject.Object, FileManager.MenuProvider):
    """A context menu for sending files via CyclopConnect."""
    ...
    def _on_g_signal(self, proxy, sender_name, signal_name, parameters):
        ...
        if signal_name == 'InterfacesAdded':
            for object_path, props in objects.items():
                props = props['org.gnome.Shell.Extensions.CyclopConnect.Device']
    ...
    def get_file_items(self, window, files):
        ...
        menu = FileManager.MenuItem(
            name='CyclopConnectShareExtension::Devices',
        ...
        for name, action_group in devices:
            item = FileManager.MenuItem(
                name='CyclopConnectShareExtension::Device' + name,
                label=name
            )
