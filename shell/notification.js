...
const _ = Extension._;
const APP_ID = 'org.gnome.Shell.Extensions.CyclopConnect';
const APP_PATH = '/org/gnome/Shell/Extensions/CyclopConnect';
...
const NotificationBanner = GObject.registerClass({
    GTypeName: 'CyclopConnectNotificationBanner',
...
const Source = GObject.registerClass({
    GTypeName: 'CyclopConnectNotificationSource',
}, class Source extends NotificationDaemon.GtkNotificationDaemonAppSource {

    _closeCyclopConnectNotification(notification, reason) {
    ...
    addNotification(notificationId, notificationParams, showBanner) {
        ...
        // Device Notification
            ...
            notification.connect('destroy', (notification, reason) => {
                this._closeCyclopConnectNotification(notification, reason);
...
/**
 * If there is an active GtkNotificationDaemonAppSource for CyclopConnect when the
 * extension is loaded, it has to be patched in place.
 */
function patchCyclopConnectNotificationSource() {
    let source = Main.notificationDaemon._gtkNotificationDaemon._sources[APP_ID];

    if (source !== undefined) {
        // Patch in the subclassed methods
        source._closeCyclopConnectNotification = Source.prototype._closeCyclopConnectNotification;
        ...
        for (let notification of Object.values(source._notifications)) {
            ...
            let _id = notification.connect('destroy', (notification, reason) => {
                source._closeCyclopConnectNotification(notification, reason);
...
/**
 * Wrap GtkNotificationDaemon._ensureAppSource() to patch CyclopConnect's app source
...
function patchGtkNotificationSources() {
    ...
    let addNotification = function (notificationId, notificationParams, showBanner) {
        ...
        notification.connect('destroy', (notification, reason) => {
            this._withdrawCyclopConnectNotification(notification, reason);
    ...
    // eslint-disable-next-line func-style
    let _withdrawCyclopConnectNotification = function (id, notification, reason) {
        if (reason !== MessageTray.NotificationDestroyedReason.DISMISSED)
            return;
    ...
    NotificationDaemon.GtkNotificationDaemonAppSource.prototype._withdrawCyclopConnectNotification = _withdrawCyclopConnectNotification;
}
...
function unpatchGtkNotificationSources() {
    ...
    delete NotificationDaemon.GtkNotificationDaemonAppSource.prototype._withdrawCyclopConnectNotification;
}
