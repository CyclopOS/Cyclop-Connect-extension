...
/**
 * Get a themed icon, using fallbacks from CyclopConnect's GResource when necessary.
 *
...
function getIcon(name) {
    if (getIcon._resource === undefined) {
        ...
        let iconPath = 'resource://org/gnome/Shell/Extensions/CyclopConnect/icons';
        let iconNames = [
 ...
 * necessary.
 *
 * @param {string} relativePath - A path relative to CyclopConnect's resource path
...
/**
 * Install the files necessary for the CyclopConnect service to run.
 */
function installService() {
    ...
    let fileManagers = [
        [`${dataDir}/nautilus-python/extensions`, 'nautilus-cyclopconnect.py'],
        [`${dataDir}/nemo-python/extensions`, 'nemo-cyclopconnect.py'],
    ];

    // WebExtension Manifests
    let manifestFile = 'org.gnome.shell.extensions.cyclopconnect.json';
    ...
    if (Config.IS_USER) {
        ...
        if (!_installResource(dbusDir, dbusFile, `${dbusFile}.in`))
            throw Error('CyclopConnect: Failed to install DBus Service');
        ...
        // File Manager Extensions
        let target = `${Config.PACKAGE_DATADIR}/nautilus-cyclopconnect.py`;
