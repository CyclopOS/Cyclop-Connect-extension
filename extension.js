...
const ServiceIndicator = GObject.registerClass({
    GTypeName: 'CyclopConnectServiceIndicator',
    ...
    _init() {
        ...
        // Cyclop Connect Settings
        this.settings = new Gio.Settings({
            settings_schema: Config.GSCHEMA.lookup(
                'org.gnome.Shell.Extensions.CyclopConnect',
                null
            ),
            path: '/org/gnome/shell/extensions/cyclopconnect/',
        });
        ...
        this._indicator.gicon = Extension.getIcon(
            'org.gnome.Shell.Extensions.CyclopConnect-symbolic'
        );
        this._indicator.visible = false;

        AggregateMenu._indicators.insert_child_at_index(this, 0);
        AggregateMenu._cyclopconnectconnect = this;
        ...
        this.deviceSection.actor.add_style_class_name('cyclopconnect-device-section');
        this.settings.bind(
            'show-indicators',
    ...
    _preferences() {
        Gio.Subprocess.new([`${Extension.path}/cyclopconnect-preferences`], 0);
    }
    ...
    _onDeviceAdded(service, device) {
        try {
            ...
            device.settings = new Gio.Settings({
                settings_schema: Config.GSCHEMA.lookup(
                    'org.gnome.Shell.Extensions.CyclopConnect.Device',
                    true
                ),
                path: `/org/gnome/shell/extensions/cyclopconnect/device/${device.id}/`,
            });
    ...
    destroy() {
        ...
        delete AggregateMenu._cyclopconnect;
        super.destroy();
    }
});
...
function init() {
    ...
    // and they're only used *by* CyclopConnect, they should be okay to leave.
    Utils.installService();

    // These modify the notification source for CyclopConnect's GNotifications and
    // need to be active even when the extension is disabled (eg. lock screen).
    // Since they *only* affect notifications from CyclopConnect, it should be okay
    // to leave them applied.
    Notification.patchCyclopConnectNotificationSource();
